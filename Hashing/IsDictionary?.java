...
public class Solution {
    public int solve(ArrayList<String> A, String B) {
        ...
        for(int i=0;i<A.size();i++){
            for(int j=i+1;j<A.size();j++){   
                if(!checkStrings(map,A.get(i),A.get(j))){
                    return(0);
                }
            }
        }
    ...
    public boolean checkStrings(HashMap<Character,Integer> map, String S1, String S2){
        int i=0,j=0,num1,num2;
        while(i<S1.length() && j<S2.length()){
            num1 = map.get(S1.charAt(i));
            num2 = map.get(S2.charAt(j));
            if(num1<num2){
                return(true);
            }
            if(num2<num1){
